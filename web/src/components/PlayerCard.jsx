import { Link } from "react-router-dom";
import { Sparkline } from "./Sparkline";
import { avgFmt } from "../lib/format";

// 홈 대시보드용 미니 선수 카드 — 이름·포지션 + 현재 타율/OPS + 미니 시계열.
export function PlayerCard({ player, data }) {
  const series = data?.series || [];
  const avg = series.length ? series[series.length - 1].cum_avg : null;
  const ops = series.length ? series[series.length - 1].cum_ops : null;

  return (
    <Link
      to={`/players?id=${player.playerId}`}
      className="flex flex-col rounded-2xl border border-gray-100 bg-white p-3.5 active:bg-lg-soft md:p-4 md:transition-shadow md:hover:shadow-md"
    >
      <div className="flex items-start justify-between gap-1">
        <div className="min-w-0">
          <p className="truncate text-[15px] font-extrabold text-lg-ink">{player.name}</p>
          <p className="text-[11px] text-gray-400">{player.position}</p>
        </div>
        <div className="text-right leading-none">
          <p className="text-lg font-black text-lg-red">{avgFmt(ops)}</p>
          <p className="mt-0.5 text-[10px] text-gray-400">OPS</p>
        </div>
      </div>

      <div className="mt-1 flex items-baseline gap-1">
        <span className="text-sm font-bold text-lg-ink">{avgFmt(avg)}</span>
        <span className="text-[10px] text-gray-400">타율</span>
        {data?.games != null && (
          <span className="ml-auto text-[10px] text-gray-300">{data.games}경기</span>
        )}
      </div>

      <div className="mt-2 h-12 md:h-16">
        {series.length ? (
          <Sparkline series={series} dataKey="cum_avg" />
        ) : (
          <div className="flex h-full items-center justify-center text-[10px] text-gray-300">
            기록 없음
          </div>
        )}
      </div>
    </Link>
  );
}

// 로딩 스켈레톤(카드 자리 유지)
export function PlayerCardSkeleton() {
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-3.5 md:p-4">
      <div className="flex items-start justify-between">
        <div className="space-y-1.5">
          <div className="h-3.5 w-16 rounded bg-gray-100" />
          <div className="h-2.5 w-10 rounded bg-gray-100" />
        </div>
        <div className="h-4 w-10 rounded bg-gray-100" />
      </div>
      <div className="mt-3 h-12 animate-pulse rounded bg-gray-50 md:h-16" />
    </div>
  );
}
